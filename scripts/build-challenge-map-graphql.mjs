 ...
 * {
 *   "challengeId": {
 *     "superblocks": ["superblock-1", "superblock-2"],
 *     "blocks": ["block-1", "block-2"],
 *     "name": "Challenge Title"
 *   }
 * }
 * 
 * Note: Challenges may appear in multiple superblocks (e.g., responsive-web-design,
 * responsive-web-design-22, full-stack-developer), so we track all associations.
 */
  ...
  for (const superblock of data.superblocks) {
    ...
    for (const block of superblock.blockObjects) {
      ...
      for (const challenge of block.challengeOrder) {
        ...
        // If challenge already exists, add this superblock/block to its associations
